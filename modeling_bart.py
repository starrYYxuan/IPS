...
from hcd_decoding import hierarchically_decoding
from SimCTG.SimCTGEncDec.SimCTGBART import simctgbart
...
def _expand_mask(mask: torch.Tensor, dtype: torch.dtype, tgt_len: Optional[int] = None):
    ...
    if len(mask.size()) == 2:
        ...
        bsz, src_len, src_len = mask.size()
        expanded_mask = mask[:, None, :, :].expand(bsz, 1, src_len, src_len).to(dtype)
    else:
        raise ValueError("Unexpected mask of size {}.".format(mask.size()))
...
class BartModel(BartPretrainedModel):
    ...
    def forward(
        ...
        decoder_outputs = self.decoder(
            ...
            encoder_attention_mask=encoder_padding_mask if encoder_padding_mask is not None else attention_mask,
...
class BartForConditionalGeneration(BartPretrainedModel):
    ...
    def hierarchically_decoding(self, **kwargs):
        return hierarchically_decoding(model=self, **kwargs)

    def diverse_contrastive_search(self, **kwargs):
        return simctgbart.SimCTGBART.diverse_contrastive_search(model=self, **kwargs)
            ...
            if margin is not None:
                ...
                if simctg:
                    ...
                    if locality_mask is not None and cross_mask is not None:
                        ...
                        loss = masked_lm_loss + alpha * locality_loss + (1 - alpha) * cross_loss
                    elif locality_mask is not None:
                        assert cosine_scores.shape == locality_mask.shape
                        locality_loss = self.calculate_locality_loss(cosine_scores, margin, locality_mask)
                        loss = masked_lm_loss + locality_loss
                    elif cross_mask is not None:
                        assert cosine_scores.shape == cross_mask.shape
                        cross_loss = self.calculate_isotropy_loss(cosine_scores, margin, cross_mask)
                        loss = masked_lm_loss + cross_loss
                    else:
                        loss = masked_lm_loss
            else:
                loss = masked_lm_loss
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs
        ...
        if encoder_padding_mask is not None:
            expand_size = attention_mask.shape[0] // encoder_padding_mask.shape[0]
            expanded_return_idx = (
                torch.arange(encoder_padding_mask.shape[0]).view(-1, 1).repeat(1, expand_size).view(-1).to(
                    encoder_padding_mask.device)
            )
